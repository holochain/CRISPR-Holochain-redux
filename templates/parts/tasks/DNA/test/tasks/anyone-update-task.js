  scenario("anyone-update-task", async (s, t) => {
    ...
    const update_task_result = await alice.call("tasks", "tasks", "update_task", {"id": create_task_result.Ok.id, "created_at": create_task_result.Ok.createdAt, "address": create_task_result.Ok.address, "task_input" : {"title":"Updated title first task", "done":"true"}})
    await s.consistency()
    ...
    const update_task_result_2 = await bob.call("tasks", "tasks", "update_task", {"id": update_task_result.Ok.id, "created_at": update_task_result.Ok.createdAt, "address": update_task_result.Ok.address, "task_input" : {"title":"Updated again title first task", "done":"true"}})
    await s.consistency()
    ...
    t.deepEqual(read_task_result.Ok.done, true)
  })