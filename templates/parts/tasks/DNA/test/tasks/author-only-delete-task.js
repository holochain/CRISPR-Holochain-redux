  scenario("author-only-delete-task", async (s, t) => {
    ...
    const deleteResult = await bob.call("tasks", "tasks", "delete_task", { "id": createNoteResult.Ok.id, "created_at": createNoteResult.Ok.createdAt, "address": createNoteResult.Ok.address })
    t.deepEqual(JSON.parse(deleteResult.Err.Internal).kind, { ValidationFailed: 'Agent who did not author is trying to delete' })
    const listNotesResult2 = await alice.call("tasks", "tasks", "list_tasks", {})
