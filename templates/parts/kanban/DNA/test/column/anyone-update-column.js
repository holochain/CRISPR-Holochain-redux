scenario("anyone-update-column", async (s, t) => {
  ...
  const update_column_result = await alice.call("columns", "columns", "update_column", {"id": create_column_result.Ok.id, "created_at": create_column_result.Ok.createdAt, "address": create_column_result.Ok.address, "column_input" : {"title":"Updated title first column", "order": 1}})
  await s.consistency()
  ...
  const update_column_result_2 = await bob.call("columns", "columns", "update_column", {"id": update_column_result.Ok.id, "created_at": update_column_result.Ok.createdAt, "address": update_column_result.Ok.address, "column_input" : {"title":"Updated again title first column", "order": 1}})
  await s.consistency()
