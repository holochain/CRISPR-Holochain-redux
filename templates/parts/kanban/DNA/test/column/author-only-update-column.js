scenario("author-only-update-column", async (s, t) => {
  ...
  const update_column_result = await bob.call("columns", "columns", "update_column", {"id": create_column_result.Ok.id, "created_at": create_column_result.Ok.createdAt, "address": create_column_result.Ok.address, "column_input" : {"uuid": create_column_result.Ok.uuid, "title":"Updated title first column", "order": 1}})
