  scenario("author-only-update-origin", async (s, t) => {
    ...
    const update_origin_result = await bob.call("origins", "origins", "update_origin", {"id": create_origin_result.Ok.id, "created_at": create_origin_result.Ok.createdAt, "address": create_origin_result.Ok.address, "origin_input" : {"uuid": create_origin_result.Ok.uuid, _updateFields}})
