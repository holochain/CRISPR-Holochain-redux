...
module.exports = (scenario, conductorConfig) => {
  scenario("create_column", async (s, t) => {
    ...
    const create_column_result = await alice.call("columns", "columns", "create_column", {"base": "testbase", "column_input" : {"uuid":uuidv4(), "title":"Title first column", "content": "Content", "order": 1}})
    await s.consistency()
    ...
    t.deepEqual(read_column_result.Ok.content, 'Content')
  })
  ...
  scenario("rebase_column", async (s, t) => {
    ...
    await s.consistency()
    console.log('create_column_result', create_column_result)
    const read_column_result = await alice.call("columns", "columns", "rebase_column", {"base_from": "testbase", "base_to": "testbase_rebase", "id": create_column_result.Ok.id, "created_at": create_column_result.Ok.createdAt})
    await s.consistency()
    const list_columns_result_1 = await alice.call("columns", "columns", "list_columns", {"base": "testbase"})
    t.deepEqual(list_columns_result_1.Ok.length, 0)
    const list_columns_result_2 = await alice.call("columns", "columns", "list_columns", {"base": "testbase_rebase"})
    t.deepEqual(list_columns_result_2.Ok.length, 1)  
  })

  scenario("list_columns", async (s, t) => {
    const {alice, bob} = await s.players({alice: conductorConfig, bob: conductorConfig}, true)
    await alice.call("columns", "columns", "create_column", {"base": "testbase", "column_input" : {"uuid":uuidv4(), "title":"Title first column", "content": "Content", "order": 1}})
	  await alice.call("columns", "columns", "create_column", {"base": "testbase", "column_input" : {"uuid":uuidv4(), "title":"Title second column", "content": "Content", "order": 1}})
    await alice.call("columns", "columns", "create_column", {"base": "testbase", "column_input" : {"uuid":uuidv4(), "title":"Title third column", "content": "Content", "order": 1}})
    await alice.call("columns", "columns", "create_column", {"base": "testbase", "column_input" : {"uuid":uuidv4(), "title":"Title fourth column", "content": "Content", "order": 1}})
    await s.consistency()
  ...
  scenario("anyone-update-column", async (s, t) => {
    const {alice, bob} = await s.players({alice: conductorConfig, bob: conductorConfig}, true)
    const create_column_result = await alice.call("columns", "columns", "create_column", {"base": "testbase", "column_input" : {"uuid":uuidv4(), "title":"Title first column", "content": "Content", "order": 1}})
    const update_column_result = await alice.call("columns", "columns", "update_column", {"id": create_column_result.Ok.id, "created_at": create_column_result.Ok.createdAt, "address": create_column_result.Ok.address, "column_input" : {"uuid": create_column_result.Ok.uuid, "title":"Updated title first column", "content": "Content", "order": 1}})
    await s.consistency()
    ...
    const update_column_result_2 = await bob.call("columns", "columns", "update_column", {"id": update_column_result.Ok.id, "created_at": update_column_result.Ok.createdAt, "address": update_column_result.Ok.address, "column_input" : {"uuid": update_column_result.Ok.uuid, "title":"Updated again title first column", "content": "Content", "order": 1}})
    await s.consistency()
