...
module.exports = (scenario, conductorConfig) => {
  scenario("create_column", async (s, t) => {
    ...
    const create_column_result = await alice.call("columns", "columns", "create_column", createParams)
    await s.consistency()
  ...
  scenario("list_columns", async (s, t) => {
    ...
    await alice.call("columns", "columns", "create_column", createParams)
	  await alice.call("columns", "columns", "create_column", createParams)
    await alice.call("columns", "columns", "create_column", createParams)
    await alice.call("columns", "columns", "create_column", createParams)
    await s.consistency()
  ...
  scenario("anyone-update-column", async (s, t) => {
    const {alice, bob} = await s.players({alice: conductorConfig, bob: conductorConfig}, true)
    const create_column_result = await alice.call("columns", "columns", "create_column", createParams)
    const update_column_result = await alice.call("columns", "columns", "update_column", {"id": create_column_result.Ok.id, "created_at": create_column_result.Ok.createdAt, "address": create_column_result.Ok.address, "column_input" : {"uuid": create_column_result.Ok.uuid, "title": "Update string for testing","order": 1}})
    await s.consistency()
    ...
    const update_column_result_2 = await bob.call("columns", "columns", "update_column", {"id": update_column_result.Ok.id, "created_at": update_column_result.Ok.createdAt, "address": update_column_result.Ok.address, "column_input" : {"uuid": update_column_result.Ok.uuid, "title": "Update string for testing","order": 1}})
    await s.consistency()
