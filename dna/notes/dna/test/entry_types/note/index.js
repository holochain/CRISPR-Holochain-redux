...
const createParams = {"base": "testbase", "note_input" : {"uuid":uuidv4(), "title": "String for testing","content": "String for testing","order": 0,"uuid": "String for testing"}}
module.exports = (scenario, conductorConfig) => {
  ...
  scenario("anyone-update-note", async (s, t) => {
    const {alice, bob} = await s.players({alice: conductorConfig, bob: conductorConfig}, true)
    const create_note_result = await alice.call("notes", "notes", "create_note", createParams)
    const update_note_result = await alice.call("notes", "notes", "update_note", {"id": create_note_result.Ok.id, "created_at": create_note_result.Ok.createdAt, "address": create_note_result.Ok.address, "note_input" : {"uuid": create_note_result.Ok.uuid, "title": "Update string for testing","content": "Update string for testing","order": 1,"uuid": "Update string for testing"}})
    await s.consistency()
    ...
    const update_note_result_2 = await bob.call("notes", "notes", "update_note", {"id": update_note_result.Ok.id, "created_at": update_note_result.Ok.createdAt, "address": update_note_result.Ok.address, "note_input" : {"uuid": update_note_result.Ok.uuid, "title": "Update string for testing","content": "Update string for testing","order": 1,"uuid": "Update string for testing"}})
    await s.consistency()
