...
const createParams = {"base": "testbase", "origin_input" : {"uuid":uuidv4(), "content": "String for testing","uuid": "String for testing"}}
module.exports = (scenario, conductorConfig) => {
  ...
  scenario("anyone-update-origin", async (s, t) => {
    const {alice, bob} = await s.players({alice: conductorConfig, bob: conductorConfig}, true)
    const create_origin_result = await alice.call("origins", "origins", "create_origin", createParams)
    const update_origin_result = await alice.call("origins", "origins", "update_origin", {"id": create_origin_result.Ok.id, "created_at": create_origin_result.Ok.createdAt, "address": create_origin_result.Ok.address, "origin_input" : {"uuid": create_origin_result.Ok.uuid, "content": "Update string for testing","uuid": "Update string for testing"}})
    await s.consistency()
    ...
    const update_origin_result_2 = await bob.call("origins", "origins", "update_origin", {"id": update_origin_result.Ok.id, "created_at": update_origin_result.Ok.createdAt, "address": update_origin_result.Ok.address, "origin_input" : {"uuid": update_origin_result.Ok.uuid, "content": "Update string for testing","uuid": "Update string for testing"}})
    await s.consistency()
