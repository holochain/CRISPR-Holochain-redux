...
orchestrator.registerScenario("Generate config and key for Alice & Bob", async (s, t) => {
  ...
  console.log('doNote1', doNote1)
  await s.consistency()
  const doNote2Task1 = await alice.call("tasks", "tasks", "create_task", {"base": doNote2.Ok.id, "task_input" : {"uuid":uuidv4(), "title":"Enable Drag & Drop", "done":true}})
  console.log('doNote2Task1', doNote2Task1)
  const doNote2Task2 = await alice.call("tasks", "tasks", "create_task", {"base": doNote2.Ok.id, "task_input" : {"uuid":uuidv4(), "title":"Update column order on drop", "done":true}})
  console.log('doNote2Task2', doNote2Task2)
  const doNote2Task3 = await alice.call("tasks", "tasks", "create_task", {"base": doNote2.Ok.id, "task_input" : {"uuid":uuidv4(), "title":"Update base on drop", "done":true}})
  console.log('doNote2Task3', doNote2Task3)
  const doNote2Task4 = await alice.call("tasks", "tasks", "create_task", {"base": doNote2.Ok.id, "task_input" : {"uuid":uuidv4(), "title":"Add new handler to move link to new base", "done":true}})
