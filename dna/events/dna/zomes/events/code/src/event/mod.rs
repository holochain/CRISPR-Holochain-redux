...
const EVENTS_ANCHOR_TYPE: &str = "list_events";
const EVENT_ENTRY_LINK_TYPE: &str = "event_link";
const EVENT_ENTRY_NAME: &str = "event";
...
impl Event {
    pub fn new(address: Address, entry: EventEntry) -> ZomeApiResult<Event> {
        Ok(Event{
            id: address.clone(),
...
impl Event {
    pub fn existing(id: Address, created_at: Iso8601, address: Address, entry: EventEntry) -> ZomeApiResult<Event> {
        Ok(Event{
            id: id.clone(),
...
pub fn definition() -> ValidatingEntryType {
    entry!(
        name: EVENT_ENTRY_NAME,
        description: "The entry with the content.",
        sharing: Sharing::Public,
        validation_package: || {
            hdk::ValidationPackageDefinition::Entry
        },
        validation: | validation_data: hdk::EntryValidationData<EventEntry>| {
        ...
        links: [
            from!(      
                ...
                link_type: EVENT_ENTRY_LINK_TYPE,
                validation_package: || {
                    hdk::ValidationPackageDefinition::Entry
                },
