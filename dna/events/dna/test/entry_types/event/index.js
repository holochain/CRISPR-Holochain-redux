...
const createParams = {"base": "testbase", "event_input" : {"uuid":uuidv4(), "content": "String for testing","uuid": "String for testing"}}
module.exports = (scenario, conductorConfig) => {
  ...
  scenario("anyone-update-event", async (s, t) => {
    const {alice, bob} = await s.players({alice: conductorConfig, bob: conductorConfig}, true)
    const create_event_result = await alice.call("events", "events", "create_event", createParams)
    const update_event_result = await alice.call("events", "events", "update_event", {"id": create_event_result.Ok.id, "created_at": create_event_result.Ok.createdAt, "address": create_event_result.Ok.address, "event_input" : {"uuid": create_event_result.Ok.uuid, "content": "Update string for testing","uuid": "Update string for testing"}})
    await s.consistency()
    ...
    const update_event_result_2 = await bob.call("events", "events", "update_event", {"id": update_event_result.Ok.id, "created_at": update_event_result.Ok.createdAt, "address": update_event_result.Ok.address, "event_input" : {"uuid": update_event_result.Ok.uuid, "content": "Update string for testing","uuid": "Update string for testing"}})
    await s.consistency()
