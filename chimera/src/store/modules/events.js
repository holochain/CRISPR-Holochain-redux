...
export default {
  ...
  actions: {
    ...
    order: ({ state, commit, rootState }, payload) => {
      ...
      payload.events.forEach(event => {
        rootState.devHolochainConnection.then(({ callZome }) => {
          callZome('events', 'events', 'update_event')({ id: event.id, created_at: event.createdAt, address: event.address, event_input: { uuid: event.uuid, title: event.title, content: event.content, order: event.order } }).then((result) => {
    ...
    rebase: ({ state, commit, rootState }, payload) => {
      rootState.devHolochainConnection.then(({ callZome }) => {
        callZome('events', 'events', 'rebase_event')({ base_from: payload.from, base_to: payload.to, id: payload.id, created_at: payload.createdAt }).then((result) => {
          const res = JSON.parse(result)
          console.log(res)
          if (res.Ok === undefined) {
            commit('error', { base: payload.base, error: res.Err.Internal })
    ...
    agentAddress: ({ state, commit, rootState, dispatch }) => {
      rootState.devHolochainConnection.then(({ callZome }) => {
        callZome('events', 'events', 'agent_address')({ }).then((result) => {
          const res = JSON.parse(result)
          console.log(res)
          if (res.Ok === undefined) {
            console.log(res)
    ...
    fetchProfiles: ({ state, commit, rootState, dispatch }) => {
      rootState.devHolochainConnection.then(({ callZome }) => {
        callZome('events', 'events', 'list_profiles')({ base: '' }).then((result) => {
          const res = JSON.parse(result)
          console.log(res.Ok)
          if (res.Ok === undefined) {
            console.log(res)
    ...
    fetchEvents: ({ state, commit, rootState }, base) => {
      ...
      rootState.devHolochainConnection.then(({ callZome }) => {
        callZome('events', 'events', 'list_events')({ base: base }).then((result) => {
          const res = JSON.parse(result)
          // console.log(res)
          if (res.Ok === undefined) {
            console.log(res)
          } else {
            commit('setEventsList', { base: base, events: res.Ok })
          }
        })
      })
    },
    saveEvent: ({ state, commit, rootState }, payload) => {
      ...
      if (payload.event.id === 'new' || payload.event.id === undefined) {
        rootState.devHolochainConnection.then(({ callZome }) => {
          callZome('events', 'events', 'create_event')({ base: payload.base, event_input: { uuid: uuidv4(), title: payload.event.title, content: payload.event.content, order: payload.event.order } }).then((result) => {
            const res = JSON.parse(result)
            console.log(res)
            if (res.Ok === undefined) {
              commit('error', { base: payload.base, error: res.Err.Internal })
        ...
        rootState.devHolochainConnection.then(({ callZome }) => {
          callZome('events', 'events', 'update_event')({ id: payload.event.id, created_at: payload.event.createdAt, address: payload.event.address, event_input: { uuid: payload.event.uuid, title: payload.event.title, content: payload.event.content, order: payload.event.order } }).then((result) => {
            const res = JSON.parse(result)
            // console.log(res)
            if (res.Ok === undefined) {
              commit('error', { base: payload.base, error: res.Err.Internal })
    ...
    deleteEvent: ({ state, commit, rootState }, payload) => {
      if (payload.base === 'PartEditor') return
      rootState.devHolochainConnection.then(({ callZome }) => {
        callZome('events', 'events', 'delete_event')({ base: payload.base, id: payload.event.id, created_at: payload.event.createdAt, address: payload.event.address }).then((result) => {
          const res = JSON.parse(result)
          if (res.Ok === undefined) {
            commit('error', { base: payload.base, error: res.Err.Internal })
          } else {
