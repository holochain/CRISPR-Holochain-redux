<template>
  <v-card class="ma-1" dark>
    ...
    <v-alert v-model="help" dismissible border="left" colored-border color="deep-purple accent-4" elevation="2">
      <div v-if="chimera">
        Hover over the <v-icon>mdi-dna</v-icon> to see which parts can be added to this PartTemplate.
        <v-divider class="my-4 info" style="opacity: 0.22" />
      </div>
      Clicks <v-icon>mdi-note-text-outline</v-icon> to edit a PartTemplate.
      <v-divider class="my-4 info" style="opacity: 0.22" />
      Click <v-icon>mdi-content-save</v-icon> to save a PartTemplate.
      <v-divider class="my-4 info" style="opacity: 0.22" />
      Click <v-icon>mdi-delete-outline</v-icon> to delete a PartTemplate.
    </v-alert>
