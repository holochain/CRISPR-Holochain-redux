  ...
  <v-card width="100%" class="fill-height" flat>
    <v-row align="center" justify="start" class="pa-1">
      ...
      <v-col cols="2" align="center">
        <v-select :items="linkContracts" v-model="selected.linkContract" :disabled="!isEditing" label="Contract" hint="Store" persistent-hint></v-select>
      </v-col>
    </v-row>
    <v-card-actions>
      <v-spacer></v-spacer>
...
export default {
  ...
  data () {
    return {
      ...
      linkContracts: ['Store', 'Request', 'Agent Agree'],
  ...
  mounted () {
    if (this.profileSpecField.fieldName !== '') {
      const id = this.profileSpecField.id
      ...
      this.selected = { index: this.index, id: id, fieldName: fieldName, linkContract: linkContract, description: description }
    } else {
      this.isEditing = true
    }
  }
