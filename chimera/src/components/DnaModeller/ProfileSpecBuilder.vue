     ...
     <v-list-item-action v-if="isEditing">
       <v-dialog v-model="dialog" persistent max-width="290">
          ...
          <v-card>
            ...
            <v-card-text>This will remove {{zome.name}}.</v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
...
export default {
  ...
  components: {
    ProfileSpecField: () => import('@/components/DnaModeller/ProfileSpecField')
  },
  props: ['zome', 'profileSpec'],
  data () {
    return {
  ...
  methods: {
    ...
    saveField (fieldIndex, field) {
      ...
      this.$emit('profile-spec-updated', this.profileSpec)
    },
    deleteField (field) {
