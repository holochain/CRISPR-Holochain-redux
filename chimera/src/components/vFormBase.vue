        ...
        <template v-slot:activator="{ on }" >
          ...
          >
            ...
            <slot :name="getTypeItemSlot(obj)">
              ...
              <!-- END BTN -->
              <!-- TipTap -->
                <div v-else-if="obj.schema.type === 'tiptap' && !editing" v-html="obj.value" />
                <tiptap-vuetify
                  v-else-if="obj.schema.type === 'tiptap' && editing"
                  :value="setValue(obj)"
                  @input="onInput($event, obj)"
                  @focus="onEvent($event, obj)"
                  @blur="onEvent($event, obj)"
              ...
              <div v-else>
                <div v-if="!editing" v-html="obj.value" :class="obj.schema.class" />
                <div
                  :is="mapTypeToComponent(obj.schema.type)"
                  v-if="editing"
                  v-bind="bindSchema(obj)"
...
const groupingType = 'group' // eslint-disable-line no-unused-vars
// symbol on drop
...
export default {
  ...
  methods: {
    ...
    onEvent (event, obj, tag) {
      ...
      delay(() => { this.emitValue(event.type, emitObj), onEventDelay }) // eslint-disable-line no-unused-expressions no-sequences
      return emitObj
    },
